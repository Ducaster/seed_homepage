/**
 * 성격유형 검사(신) — 에니어그램 기반 99문항
 *
 * 9개 그룹 × (10문항 + 1역채점※) = 99문항
 * 그룹 순서 → 유형 매핑: 8, 9, 1, 2, 3, 4, 5, 6, 7
 */

export interface PersonalityQuestion {
  groupIndex: number; // 0-8
  questionIndex: number; // 0-10 (10 = ※ 역채점)
  text: string;
  isReverse: boolean;
}

export const TYPE_ORDER = [8, 9, 1, 2, 3, 4, 5, 6, 7] as const;
...
export const PERSONALITY_QUESTIONS: PersonalityQuestion[] = raw.flatMap(
  (group, groupIndex) =>
    group.map((text, questionIndex) => ({
      groupIndex,
      questionIndex,
      text,
      isReverse: questionIndex === 10,
    }))
);

export const QUESTIONS_PER_GROUP = 11;
